...
class EventService {
    ...
    public function createEventInboxItemFromEmbed($payload)
    {
        ...
        foreach($users as $user) {
            if(!in_array('EVENTS_INBOX', $user->getNotifications() ?? [])) {
                continue;
            }

            $this->userService->sendNotification(
                $user,
                'Eine neue Veranstaltung wurde über die Website eingereicht',
                sprintf('%s', $payload['title']),
                sprintf(
                    '<p>%s hat eine neue Veranstaltung eingereicht.</p><p>Prüfen Sie den Posteingang um den Eintrag freizuschalten.</p><p>Kontaktdaten für Rückfragen: %s / %s</p>',
